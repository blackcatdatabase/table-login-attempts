...
use BlackCat\Database\Packages\LoginAttempts\Dto\LoginAttemptDto as Dto;
use BlackCat\Database\Packages\LoginAttempts\Mapper\LoginAttemptDtoMapper as RowMapper;
...
final class LoginAttemptRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\LoginAttempts\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\LoginAttempts\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\LoginAttempts\Dto\LoginAttemptDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\LoginAttempts\Dto\LoginAttemptDto|null {
        $row = $this->getByUnique([ 'id' => $id ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\LoginAttempts\Mapper\LoginAttemptDtoMapper::fromRow($row);
    }
