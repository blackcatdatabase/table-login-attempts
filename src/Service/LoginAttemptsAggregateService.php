...
namespace BlackCat\Database\Packages\LoginAttempts\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\LoginAttempts\Dto\LoginAttemptDto;
use BlackCat\Database\Packages\LoginAttempts\Mapper\LoginAttemptDtoMapper;
 ...
 */
final class LoginAttemptsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private LoginAttemptRepository $loginAttemptRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'login_attempts' => $this->loginAttemptRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\LoginAttempts\ModuleException("Record {$idStr} not found for locking.");
            }
