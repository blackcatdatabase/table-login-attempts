...
namespace BlackCat\Database\Packages\LoginAttempts\Joins;
 ...
 */
final class LoginAttemptsJoins {
    ...
    /**
     * FK: login_attempts -> auth_events
     * LEFT JOIN vw_auth_events AS $as ON $as.id = $alias.auth_event_id
     * @return array{0:string,1:array<string,mixed>}
     */
    public function joinAuthEvents(string $alias = 't', string $as = 'j1'): array {
        [$alias, $as] = $this->assertAliasPair($alias, $as);
        return [' LEFT JOIN vw_auth_events AS ' . $as . ' ON ' . $as . '.id = ' . $alias . '.auth_event_id' . ' ', []];
    }
